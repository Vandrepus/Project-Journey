...
<body class="bg-gray-100 min-h-screen flex flex-col">
  ...
  <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="bg-white shadow-lg rounded-lg p-6">
      ...
      <!-- Replies Sorting Form -->
      <div class="mb-4 flex items-center">
        <label for="sort_replies" class="text-sm font-medium text-gray-700 mr-2">Sort replies by:</label>
        <form method="GET" action="{{ route('forum.show', $topic->id) }}" class="flex items-center">
          <select name="sort_replies" id="sort_replies" class="select select-bordered">
            <option value="newest" {{ request('sort_replies', 'newest') == 'newest' ? 'selected' : '' }}>Newest</option>
            <option value="oldest" {{ request('sort_replies') == 'oldest' ? 'selected' : '' }}>Oldest</option>
          </select>
          <!-- Preserve any existing query parameters such as search -->
          <input type="hidden" name="search" value="{{ request('search') }}">
          <button type="submit" class="btn btn-primary ml-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Sort
          </button>
        </form>
      </div>
      ...
      <div class="bg-gray-50 p-6 rounded-lg shadow-inner">
        ...
        @php
          $sortOrder = request('sort_replies', 'newest');
          $sortedReplies = $topic->replies;
          if($sortOrder === 'oldest'){
              $sortedReplies = $topic->replies->sortBy('created_at');
          } else {
              $sortedReplies = $topic->replies->sortByDesc('created_at');
          }
        @endphp
                ...
                <div class="mt-4 flex items-center space-x-4">
                  @auth
                    ...
                    @if (!auth()->user()->isAdmin() && auth()->id() !== $reply->user_id && !$reply->user->isAdmin())
                      <button type="button" onclick="openReportModal({{ $reply->id }})" class="btn btn-warning px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600">
        ...
        <div class="mt-8">
          ...
          <form action="{{ route('forum.reply', $topic->id) }}" method="POST" class="space-y-6">
            ...
            <div>
              ...
              <textarea name="content" id="content" rows="5" maxlength="200" oninput="updateReplyCharacterCount(event)" class="textarea textarea-bordered w-full focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Write your reply here..." required></textarea>
    ...
    <div class="bg-white w-full max-w-md p-6 rounded-lg shadow-lg">
      ...
      <form id="reportForm" method="POST" action="{{ route('reports.store') }}">
        ...
        <div class="form-control mb-4">
          ...
          <textarea name="reason" id="reason" rows="4" class="textarea textarea-bordered w-full" placeholder="Explain why you're reporting this reply..." required></textarea>
        </div>
        <div class="flex justify-end space-x-4">
          <button type="button" onclick="closeReportModal()" class="btn btn-secondary px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400">Cancel</button>
          <button type="submit" class="btn btn-primary px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Submit Report</button>
        </div>
      </form>
    </div>
  </div>
